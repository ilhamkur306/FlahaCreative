      ...
      <div className="max-w-7xl mx-auto px-4 w-full h-full flex items-center">
        <div className="grid lg:grid-cols-2 gap-8 items-start w-full">
          ...
          <div>
            ...
            <div className="w-16 h-1 bg-white/80 mb-4"></div>
            ...
            <div className="space-y-4 mb-6">
              {contactInfo.map((contact, index) => (
                <div key={index} className="flex items-start space-x-3">
                  <div className="bg-white/20 p-2 rounded-lg text-white flex-shrink-0 text-sm">
                    {contact.icon}
                  </div>
                  <div>
                    <h3 className="text-sm font-semibold mb-1 text-white">{contact.title}</h3>
                    <p className="text-base font-bold mb-1 text-white">{contact.info}</p>
                    <p className="text-white/80 text-xs">{contact.description}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Social Media */}
            <div>
              <h3 className="text-sm font-semibold mb-3 text-white">Ikuti Kami</h3>
              <div className="flex space-x-3">
                {socialMedia.map((social, index) => (
                  <a
                    key={index}
                    href={social.url}
                    className="bg-white/20 p-2 rounded-lg hover:bg-white/30 transition-all duration-300 transform hover:scale-110 text-sm"
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
            <h3 className="text-lg font-bold mb-4 text-white">Konsultasi Gratis</h3>
            <form onSubmit={handleSubmit} className="space-y-3">
              <div className="grid md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium mb-1 text-white">Nama Lengkap</label>
                  <input
                    type="text"
                    name="name"
                    value={formData.name}
                    onChange={handleInputChange}
                    className="w-full px-3 py-2 rounded-lg bg-white/20 border border-white/30 text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-transparent text-sm"
                    placeholder="Masukkan nama Anda"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium mb-1 text-white">No. Telepon</label>
                  <input
                    type="tel"
                    name="phone"
              ...
              <div>
                <label className="block text-xs font-medium mb-1 text-white">Jenis Layanan</label>
                <select 
                  name="eventType"
                  value={formData.eventType}
                  onChange={handleInputChange}
                  className="w-full px-3 py-2 rounded-lg bg-white/20 border border-white/30 text-white focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-transparent text-sm"
                >
              ...
              <div>
                <label className="block text-xs font-medium mb-1 text-white">Pesan</label>
                <textarea
                  name="message"
                  value={formData.message}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-3 py-2 rounded-lg bg-white/20 border border-white/30 text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-transparent resize-none text-sm"
              ...
              <button
                type="submit"
                className="w-full bg-white text-[#103641] py-2 px-4 rounded-lg font-bold text-sm hover:bg-white/90 transition-all duration-300 transform hover:scale-105 shadow-lg"
              >
                Kirim Pesan
              </button>
            </form>
            ...
            <div className="mt-4 pt-4 border-t border-white/20">
              <div className="text-center">
                <p className="text-white/80 mb-2 text-xs">Atau hubungi langsung via WhatsApp</p>
                <a
                  href="https://wa.me/6282229914755"
                  className="inline-flex items-center bg-green-500 text-white px-4 py-2 rounded-lg font-semibold hover:bg-green-600 transition-all duration-300 transform hover:scale-105 text-xs"
                >
